...
import { X01PropsForSelects } from '../Classes/X01';
import { ListWithDataArrays } from '../../Interfaces/General';

class X01 extends React.Component<any, any> {
    componentWillMount(){
        const list: X01PropsForSelects = new X01PropsForSelects()
        list.data = new ListWithDataArrays()
        list.data.scores = maxScoresList.map((number: number, index: number) => { return (<option key={index} value={number}>{number}</option>) })
        list.data.checkouts = checkOutList.map((number: string, index: number) => { return (<option key={index} value={number}>{number}</option>) }) || null,
        list.data.levels = levelList.map((level: number) => { return (<option key={level + 1} value={level + 1}>Level {level + 1}</option>) }) || null,
        list.data.sets = setList.map((set: number) => { return (<option key={set + 1} value={set + 1}>{set + 1}</option>) }) || null,
        list.data.legs = legsList.map((leg: number) => { return (<option key={leg + 1} value={leg + 1}>{leg + 1}</option>) }) || null,
